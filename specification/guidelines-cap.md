...
- Frontend app lives in `assets/<asset-name>/ui/` and is compiled during build to `assets/<asset-name>/app/` (when a React UI is required)
- A placeholder `app/index.html` is always created so the CAP preview serves a page even without a full UI
- The `cap-development` skill handles project init, modeling, testing, and optional frontend scaffolding
